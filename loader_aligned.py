...
class DataLoader:
    ...
    def __init__(self, data_file, align_file, max_word=128):

        self.max_word = max_word
        self.dataset = load_pkl(data_file)[1000:]
        self.align = load_pkl(align_file)[1000:]
        assert len(self.dataset) == len(self.align)
    ...
    def sampled_batch(self, batch_size, phase='train'):
        ...
        while self.pos < n:
            ...
            for i in range(batch_size):
                ...
                align = self.align[index[self.pos]]
                ...
                Align_1.append(align[0])
                Align_2.append(align[1])
                self.pos += 1
                if self.pos >= n:
                    break

            yield Index, array(X1_batch), array(X2_batch), array(M1_batch), array(M2_batch), array(Y_batch),\
                  array(P1_batch), array(P2_batch), array(Align_1), array(Align_2)
