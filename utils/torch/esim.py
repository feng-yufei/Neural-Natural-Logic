...
def sort_by_seq_lens(batch, sequences_lengths, descending=True):
    ...
    #print(reverse_mapping.device, idx_range.device)
    restoration_index = idx_range.index_select(0, reverse_mapping.cpu())

    return sorted_batch, sorted_seq_lens, sorting_index, restoration_index


def get_mask_by_sequence_length(sequences_batch, sequences_lengths):
    """
...
class Seq2SeqEncoder(nn.Module):
    ...
    def forward(self, sequences_batch, sequences_lengths):
        ...
        #print(sorted_batch.device, sorted_lengths.device)
        packed_batch = nn.utils.rnn.pack_padded_sequence(sorted_batch.cpu(),
                                                         sorted_lengths.cpu(),
                                                         batch_first=True).cuda()

        outputs, (last_hidden, _) = self._encoder(packed_batch, None)
        # n_layer* n_direction, batch_size, hidden
        #print(last_hidden.device)
        last_hidden = last_hidden.permute(1, 0, 2).reshape(last_hidden.shape[1], -1)

        outputs, _ = nn.utils.rnn.pad_packed_sequence(outputs,
                                                      batch_first=True)
        restoration_idx = torch.tensor(restoration_idx).cuda()
        #print(restoration_idx.device)
        reordered_outputs = outputs.index_select(0, restoration_idx)
        last_hidden = last_hidden.index_select(0, restoration_idx)

        return reordered_outputs, last_hidden
...
class SoftmaxAttention(nn.Module):
    ...
    def forward(self,
        ...
        if attention_matrix:
            return attended_premises, attended_hypotheses, hyp_prem_attn
        else:
