...
class NNL_Aligned(nn.Module):
    def __init__(self, embed_model, hidden_size, padding_idx=0, dropout=0.5, dropout_relnet=0.0, num_classes=3, num_heads=3):
        ...
        self.memory_key_layer = nn.Sequential(nn.Linear(4 * hidden_size, hidden_size), nn.Tanh(),
                                              nn.Linear(hidden_size, hidden_size))
        self.memory_states_layer = nn.Sequential(nn.Linear(4 * hidden_size, hidden_size), nn.Tanh(),
                                              nn.Linear(hidden_size, hidden_size))
        self.gate_layer = nn.Sequential(nn.Linear(hidden_size, 49), nn.Sigmoid())
        #self.gate_layer_fwd = nn.Sequential(nn.Linear(hidden_size, 1))
        #self.gate_layer_negaltcov = nn.Sequential(nn.Linear(hidden_size, 3))
    ...
    def forward(self,
        ...
        if not basic_rel:
            ...
            memory_key = self.memory_key_layer(torch.cat([premise_aligned, encoded_hypothesis], dim=-1))
            memory_states = self.memory_states_layer(torch.cat([premise_aligned, encoded_hypothesis], dim=-1))
            ...
            if method == "sum":
                union_1 = self.compute_union_matrix_sum(projected_rel_1, memory_key=memory_key, memory_states=memory_states)
    ...
    def compute_union_matrix_sum(self, state_batch, memory_key, memory_states):
        # state_batch: (bs, sent_len, 7)
        # memory_key: (bs, sent_len, dim)
        # memory_states: (bs, sent_len, dim)
        ...
        mem_score = torch.bmm(memory_key, memory_states.transpose(1, 2))  # bs, sent_len(key), sent_len(states)
        memory_mask = torch.ones_like(mem_score[0]).triu(diagonal=1).repeat(mem_score.shape[0], 1, 1)
        mem_score = torch.softmax(mem_score - 1e06*memory_mask, dim=-1)
        mem_emb = torch.bmm(mem_score, memory_states)  # (bs, sent_len, dim)
        mem_gate = self.gate_layer(mem_emb)  # (bs, sent_len, 49)
        ...
        for j in range(1, state_batch.shape[1]):
            ...
            for l in range(class_dim):
                this_output = torch.cat([this_output, torch.sum(output_times[:, UNION_ADD_DICT[l]] * mem_gate[:, j, UNION_ADD_DICT[l]], dim=-1)], -1)
