...
class RelNet(nn.Module):
    def __init__(self, d, dropout=0.0, num_heads=3):
        ...
        self.num_heads = num_heads
        self.units = nn.ModuleList([RelUnit(d, dropout, num_heads) for _ in range(NUM_REL)])
        # self.eq = RelUnit(d)
    ...
    def forward(self, embed_as, embed_bs, label_ids=None):
        """
        ...
        logits = torch.stack([unit(embed_as, embed_bs) for i, unit in enumerate(self.units)]).t()
