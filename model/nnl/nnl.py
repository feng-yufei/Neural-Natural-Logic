...
# {'eq': 1, 'ent_f': 2, 'ent_r': 3, 'neg': 4, 'alt': 5, 'cov': 6, 'ind': 0}
...
class NNL(nn.Module):
    ...
    def forward(self,
        ...
        if not basic_rel:
            ...
            hypothesis_project_1 = hypothesis_project[:, 0:max(hypothesis_lengths), :].contiguous().view(-1,
                                                                                                       hypothesis_project.shape[-1])
            ...
            if label_ids is not None:
                ...
                # ce loss
                loss_ce = loss_fct(logits, label_ids.view(-1))
                # margin loss
